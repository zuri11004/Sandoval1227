 ...
 % Diseño del Filtro del filtro pasabajass
    ...
    [b,a]=cheby2(nx,Rs,Wnx, 'low'); %Filtro Cheb tipo 2 
     ...
     %_______________Se carga la base de datos de entrenamiento
       ...
       load('KEIRN_testing_rotation'); % se carga la matriz original con sus etiquetas 
       %load('KEIRN_testing_counting');%nase partid por dias
     ...
     %____________hasta aqui
      ...
      recortado=testing; %Estructura de la base de datos de conjunto i para el testing
      %recortado=testing;
      [~,c] = size(recortado);
...
%            end
  ...
  nombre2=cat(2,'Matriz_norm_keirns_testing_',tarea);
  %nombre2=cat(2,'Matriz_norm_keirns_testing_dia_',tarea);
  save(nombre2,'data');
