...
%creado: 25 de Abril del 2019.
%Programa que crea una matriz de caracteristicas estadisticas de la
 ...
 % Diseño del Filtro del filtro pasabajass
    ...
    [b,a]=cheby2(nx,Rs,Wnx, 'low'); %Filtro Cheb tipo 2 
     ...
     %_______________Se carga la base de datos de entrenamiento
       ...
       load('KEIRN_training_rotation'); % se carga la matriz original con sus etiquetas 
       %load('KEIRN_training_counting');%nase partid por dias
       %load('KEIRN_trainingc1_letter_composing');
       %load('KEIRN_trainingc1_multiplication'); % se carga la matriz original con sus etiquetas
       %load('KEIRN_trainingc1_baseline');
       %load('KEIRN_trainingc1_counting');
     %____________hasta aqui
      ...
      recortado=training; %Estructura de la base de datos de conjunto i para el training
      %recortado=training;
      [~,c] = size(recortado);
...
%            end
     ...
     [data,pre_param]=Normal(Matriz_carac_final);
     
  fprintf('Tareas: \n letter-composing \n baseline\n');
  fprintf(' multiplication \n rotation \n counting\n');
  tarea = input('escribe ci_tarea');
  nombre1=cat(2,'pre_param_',tarea);
  %nombre1=cat(2,'pre_param_dia_',tarea);
  save(nombre1,'pre_param');
  nombre2=cat(2,'Matriz_norm_keirns_training_',tarea);
  %nombre2=cat(2,'Matriz_norm_keirns_training_dia_',tarea);
  save(nombre2,'data');
 ...
 function [pre_X,pre_param]=Normal(X) %se debe cancelar
%normaliza los datoss

 [n1,m1]=size(X); %tamañano de lasfilas nada mas, anula columnas
 class=X(:,m1);
 X=X(:,1:(m1-1));
